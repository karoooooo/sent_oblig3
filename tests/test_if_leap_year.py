...
def test_year_is_divisible_by_4():
    division = is_leap_year(test_year_1) % 4
    assert division, f'\n{test_year_1} was not divisible by 4'


def test_year_is_not_divisible_by_100():
    division = is_leap_year(test_year_1) % 100
    assert not division, f'\n{test_year_1} was divisible by 100'


def test_year_is_divisible_by_400():
    division = is_leap_year(test_year_1) % 400
    assert division, f'\n{test_year_1} was not divisible by 400'


def test_year_is_leap_year():
    is_it_leap_year = is_leap_year(test_year_1)
    assert is_it_leap_year, f'\n{test_year_1} is not a leap year'


def test_year_is_not_a_leap_year():
    is_it_leap_year = is_leap_year(test_year_1)
    assert not is_it_leap_year, f'\n{test_year_1} is in fact a leap year...'
